...
export default function HorseRacing() {
  ...
  const [horses, setHorses] = useState<Horse[]>([
    { id: 'alien', name: 'Alien Horse', image: '/game/alien_horse.webp', odds: 3, position: 0, speed: 1.2, lane: 0, lapsCompleted: 0 },
    { id: 'evil', name: 'Evil Horse', image: '/game/evil_horse.webp', odds: 3, position: 0, speed: 0.9, lane: 1, lapsCompleted: 0 },
    { id: 'mcqueen', name: 'McQueen Horse', image: '/game/mcqueen_horse.webp', odds: 3, position: 0, speed: 1.0, lane: 2, lapsCompleted: 0 }
  ])
    ...
    <div className="min-h-screen flex flex-col bg-[#1a472a] relative overflow-hidden">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 relative">
        ...
        <div className="fixed left-4 md:left-8 top-1/4 w-48 md:w-64 bg-black/20 rounded-lg p-2 md:p-4 border border-[#ffd700]/10">
          <h2 className={`text-lg md:text-xl text-white mb-2 md:mb-4 ${daydream.className}`}>Leaderboard</h2>
          <div className="space-y-1 md:space-y-2">
            {getSortedHorses().map((horse, index) => (
              ...
              >
                <span className={`text-base md:text-lg ${daydream.className} ${
                ...
                <div className="flex-1">
                  <p className={`text-xs md:text-sm ${daydream.className} text-white`}>{horse.name}</p>
                  <p className="text-[10px] md:text-xs text-white/60">Lap {horse.lapsCompleted + 1}/3</p>
                </div>
              </div>
            ))}
          </div>
